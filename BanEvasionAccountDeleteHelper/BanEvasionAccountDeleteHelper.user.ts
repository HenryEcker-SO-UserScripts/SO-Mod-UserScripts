...
(function () {
    ...
    function fetchFullUrlFromUserId(userId: number): Promise<string> {
    ...
    function getUserPii(userId: number): Promise<{
        email: string;
        name: string;
        ip: string;
    }> {
    ...
    function deleteUser(userId: number, deletionDetails: string) {
        const fd = new FormData();
        fd.set('fkey', StackExchange.options.user.fkey);
    ...
    function annotateUser(userId: number, annotationDetails: string) {
        const fd = new FormData();
        fd.set('fkey', StackExchange.options.user.fkey);
    ...
    function attachAttrs<T>(e: JQuery<T>, attrs: Record<string, unknown>): JQuery<T> {
    ...
    function buildLabel(text: string, attrs?: Record<string, unknown>): JQuery<HTMLLabelElement> {
        return attachAttrs(
    ...
    function buildInput(attrs: Record<string, unknown>): JQuery<HTMLInputElement> {
        return attachAttrs(
            $('<input class="s-input"/>'),
            attrs
        );
    ...
    function buildButton(text: string, attrs?: Record<string, unknown>): JQuery<HTMLButtonElement> {
        return attachAttrs(
    ...
    class DeleteEvasionAccountControls {
        ...
        private createMainAccountInput() {
            ...
            checkButton.on('click', (ev) => {
                ...
                this.mainAccountId = input.val() as number;
