...
// @version      0.0.6
// @downloadURL  https://github.com/HenryEcker/SO-Mod-UserScripts/raw/master/Delete-UndeletedFromUserPage/Delete-UndeleteFromUserPage.user.js
// @updateURL    https://github.com/HenryEcker/SO-Mod-UserScripts/raw/master/Delete-UndeletedFromUserPage/Delete-UndeleteFromUserPage.user.js
//
...
(function () {
    ...
    function castVote(postId, voteType) {
    ...
    function makeBtnClickHandler(postId, voteType, jSummary, jSummaryParent, btn, updatePostStyle) {
        return (ev) => {
            ev.preventDefault();
            void castVote(postId, voteType)
                .then((res) => {
                if (res.status === 200) {
                    // Do something to change the style of the divs
                    updatePostStyle();
                    // Replace with a new button of opposite type
                    btn.replaceWith(makeButton(toggleVoteType(voteType), postId, jSummary, jSummaryParent));
                }
            });
        };
    }
    ...
    function main() {
        ...
        for (const summary of $('.s-post-summary--stats.js-post-summary-stats')) {
            ...
            jSummary.append(makeButton(isPostDeleted ? config.undeleteVoteCode : config.deleteVoteCode, jSummaryParent.attr('data-post-id'), jSummary, jSummaryParent));
        }
    }
    StackExchange.ready(() => {
        // Run Function
